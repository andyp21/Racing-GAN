...
for epoch in range(opt.n_epochs):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        #g_loss1 = g_v1
        #g_loss2 = g_v2
        # g_v2 + torch.max(Tensor(0), g_v2 - g_v2)
        ...
        print(
            ...
            % (epoch, opt.n_epochs, i, len(dataloader), d_loss.item(), g_loss1.item(), g_loss2.item())
        )
        
        batches_done = epoch * len(dataloader) + i
        if batches_done % opt.sample_interval == 0:
          save_image(gen_imgs1.data[:25], "imagest1/%d.png" % (count/10), nrow=5, normalize=True)
          save_image(gen_imgs2.data[:25], "imagest2/%d.png" % (count/10), nrow=5, normalize=True)
